...
void printListCostum(struct Costum costums[], int a){
	...
	for (i = 0; i < a; i++) {
     	printf("| %-10d | %-25s | %-23s | %-18s |\n", 
                costums[i].idCostum, 
                costums[i].nameCostum, 
                costums[i].emailCostum, 
                costums[i].phoneCostum);
        printf("|------------|---------------------------|-------------------------|--------------------|\n");
    }
}

////them khach hang
void addCostum(struct Costum costums[],  int *a){
	system("cls");
	getchar(); 
	printf("************ Add A New Costum ************\n");
	costums[*a].idCostum=*a+1;
	printf("Moi ban nhap vao ten khach hang: ");
	fgets(costums[*a].nameCostum, 100, stdin);
	costums[*a].nameCostum[strcspn(costums[*a].nameCostum, "\n")] = '\0';
	printf("Moi ban nhap vao email: ");
	fgets(costums[*a].emailCostum, 50, stdin);
	costums[*a].emailCostum[strcspn(costums[*a].emailCostum, "\n")] = '\0';
	printf("Moi ban nhap vao so dien thoat: ");
	fgets(costums[*a].phoneCostum, 100, stdin);
	costums[*a].phoneCostum[strcspn(costums[*a].phoneCostum, "\n")] = '\0';
	(*a)++;
}
